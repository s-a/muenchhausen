...
var i18nFilename = function(culture){
  var result = path.join(__dirname, "..", "date", "i18n", culture + ".json");
  if (!fs.existsSync(result)){
    result = path.join(__dirname, "..", "date", "i18n", "en-GB.json");
  }
  return result;
}
...
Time.prototype.past = function past(context) {
  ...
  if (this.value.getHours() >= now.getHours()){
    var rest = 24 - now.getHours();
    var rnd = -1 * Math.floor(Math.random() * rest) + now.getHours()
    this.value.setHours(rnd);
  }
