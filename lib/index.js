...
Muenchhausen.prototype.initialize = function(context) {
	this.fake = new Fake();
	var self = this; 
	this.eachModuleFunction(this.fake, context, function(fn, f){
		var f = fn.replace(/\./g, "-").toLowerCase();

		Handlebars.registerHelper(f, self.handlebarsHelper);
		Handlebars.registerHelper(f + "-text", self.handlebarsHelper);
		Handlebars.registerHelper(f + "-value", self.handlebarsHelper);
	});   
};
